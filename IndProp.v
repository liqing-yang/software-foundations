...
(** We've already seen two ways of stating a proposition that a number
    ...
    even if we can _establish_ its evenness from the following rules:
       ...
       - Rule [eveni_0]: The number [0] is even.
       - Rule [eveni_SS]: If [n] is even, then [S (S n)] is even. *)

(** To illustrate how this new definition of evenness works,
    let's imagine using it to show that [4] is even. By rule [eveni_SS],
    it suffices to show that [2] is even. This, in turn, is again
    guaranteed by rule [eveni_SS], as long as we can show that [0] is
    even. But this last fact follows directly from the [eveni_0] rule. *)
...
(** We will see many definitions like this one during the rest
    ...
    use [eveni] for the name of this property, since [even] is already
    used.)
...
(** Each of the textual rules that we started with is
    ...
    [eveni_SS] says that, if [n] satisfies [eveni], then [S (S n)] also
...
| eveni_0 : eveni 0
| eveni_SS (n : nat) (H : eveni n) : eveni (S (S n)).
...
(** This definition is interestingly different from previous uses of
    ...
    [eveni] appears to the _right_ of the colon on the first line, it
    is allowed to take different values in the types of different
    constructors: [0] in the type of [eveni_0] and [S (S n)] in the type
    of [eveni_SS].  Accordingly, the type of each constructor must be
    specified explicitly (after a colon), and each constructor's type
    must have the form [eveni n] for some natural number [n].
   ...
   of the colon in defining [eveni], we would have seen an error: *)

Fail Inductive wrong_eveni (n : nat) : Prop :=
| wrong_eveni_0 : wrong_eveni 0
| wrong_eveni_SS (H: wrong_eveni n) : wrong_eveni (S (S n)).
(* ===> Error: Last occurrence of "[wrong_eveni]" must have "[n]"
        as 1st argument in "[wrong_eveni 0]". *)
...
(** In an [Inductive] definition, an argument to the type constructor
    ...
    parameter; in [Inductive eveni : nat -> Prop := ...], the unnamed
    [nat] argument is an index. *)

(** We can think of this as defining a Coq property [eveni : nat ->
    Prop], together with "evidence constructors" [eveni_0 : eveni 0]
    and [eveni_SS : forall n, eveni n -> eveni (S (S n))]. *)
...
(** These evidence constructors can be thought of as "primitive
    ...
    constructor names to obtain evidence for [eveni] of particular
    numbers... *)

Theorem eveni_4 : eveni 4.
Proof. apply eveni_SS. apply eveni_SS. apply eveni_0. Qed.

(** ... or we can use function application syntax to combine several
    constructors: *)

Theorem eveni_4' : eveni 4.
Proof. apply (eveni_SS 2 (eveni_SS 0 eveni_0)). Qed.
...
Proof.
  ...
  apply eveni_SS. apply eveni_SS. apply Hn.
Qed.

(** **** Exercise: 1 star, standard (eveni_double) *)
Theorem eveni_double : forall n,
  eveni (double n).
Proof.
  intros n. Search (double _). rewrite -> double_plus. induction n.
  - apply eveni_0.
  - simpl. rewrite <- plus_n_Sm. apply (eveni_SS (n+n) IHn).
Qed.
(** [] *)
...
(** Besides _constructing_ evidence that numbers are even, we can also
    ...
    Introducing [eveni] with an [Inductive] declaration tells Coq not
    only that the constructors [eveni_0] and [eveni_SS] are valid ways to
    build evidence that some number is [eveni], but also that these two
    constructors are the _only_ ways to build evidence that numbers
    are [eveni]. *)

(** In other words, if someone gives us evidence [E] for the assertion
    [eveni n], then we know that [E] must be one of two things:

      - [E] is [eveni_0] (and [n] is [O]), or
      - [E] is [eveni_SS n' E'] (and [n] is [S (S n')], where [E'] is
        evidence for [eveni n']). *)

(** This suggests that it should be possible to analyze a
    hypothesis of the form [eveni n] much as we do inductively defined
...
(** Suppose we are proving some fact involving a number [n], and
    we are given [eveni n] as a hypothesis.  We already know how to
    ...
    instead want to analyze the evidence that [eveni n] _directly_. As
    a tool, we can prove our characterization of evidence for
    [eveni n], using [destruct]. *)

Theorem eveni_inversion :
  forall (n : nat), eveni n ->
    (n = 0) \/ (exists n', n = S (S n') /\ eveni n').
Proof.
  intros n E.
  destruct E as [ | n' E'] eqn:EE.
  - (* E = eveni_0 : eveni 0 *)
    left. reflexivity.
  - (* E = eveni_SS n' E' : eveni (S (S n')) *)
...
(** Facts like this are often called "inversion lemmas" because they
    ...
    Here, there are two ways to prove that a number is [eveni], and
    the inversion lemma makes this explicit. *)

(** The following theorem can easily be proved using [destruct] on
    evidence. *)

Theorem eveni_minus2 : forall n,
  eveni n -> eveni (pred (pred n)).
Proof.
  intros n E.
  destruct E as [| n' E'] eqn:EE.
  - (* E = eveni_0 *) simpl. apply eveni_0.
  - (* E = eveni_SS n' E' *) simpl. apply E'.
Qed.

(** However, this variation cannot easily be handled with just
    [destruct]. *)

Theorem evSS_eveni : forall n,
  eveni (S (S n)) -> eveni n.
(** Intuitively, we know that evidence for the hypothesis cannot
    consist just of the [eveni_0] constructor, since [O] and [S] are
    different constructors of the type [nat]; hence, [eveni_SS] is the
...
(** What happened, exactly?  Calling [destruct] has the effect of
    ...
    of [eveni_minus2] because that argument [n] is mentioned directly
    in the final goal. However, it doesn't help in the case of
    [evSS_eveni] since the term that gets replaced ([S (S n)]) is not
    mentioned anywhere. *)

(** If we [remember] that term [S (S n)], the proof goes
    through.  (We'll discuss [remember] in more detail below.) *)

Theorem evSS_eveni_remember : forall n,
  eveni (S (S n)) -> eveni n.
Proof.
...
Check eveni_inversion.

Theorem evSS_eveni : forall n, eveni (S (S n)) -> eveni n.
Proof.
 intros n H. apply eveni_inversion in H.
 destruct H as [H0|H1] eqn:EE.
 - discriminate H0.
 - destruct H1 as [n' [Hnm Hev]] eqn:EE1. injection Hnm as Heq.
   rewrite Heq. apply Hev.
Qed.

(** Note how both proofs produce two subgoals, which correspond
    to the two ways of proving [eveni].  The first subgoal is a
    ...
    [eveni_SS] case must be the same as [n].  It has an "[as]" variant
...
Proof.
  ...
  (* We are in the [E = eveni_SS n' E'] case now. *)
  apply E'.
Qed.
...
Theorem one_not_even : ~ eveni 1.
Proof.
  intros H. apply eveni_inversion in H.
  destruct H as [ | [m [Hm _]]].    - discriminate H.
  - discriminate Hm.
Qed.
...
Proof.
  intros n E. inversion E as [| n' E' Heq]. inversion E'. apply H0. 
Qed.
(** [] *)
...
(** The [eveni_double] exercise above shows that our new notion of
    evenness is implied by the two earlier ones (since, by
    [even_bool_prop] in chapter [Logic], we already know that
    those are equivalent to each other). To show that all three
    coincide, we just need the following lemma. *)

Lemma eveni_evend_firsttry : forall n,
  eveni n -> evend n.
Proof.
  (* WORKED IN CLASS *)

(** We could try to proceed by case analysis or induction on [n].  But
    since [eveni] is mentioned in a premise, this strategy would
...
(** Unfortunately, the second case is harder.  We need to show [exists
    ...
    one that involves a _different_ piece of evidence for [eveni]:
...
(** The behavior of [induction] on evidence is the same as its
    ...
    To prove a property of [n] holds for all numbers for which [eveni
    n] holds, we can use induction on [eveni n]. This requires us to
    prove two things, corresponding to the two ways in which [eveni n]
    could have been constructed. If it was constructed by [eveni_0], then
    [n=0], and the property must hold of [0]. If it was constructed by
    [eveni_SS], then the evidence of [eveni n] is of the form [eveni_SS n'
    E'], where [n = S (S n')] and [E'] is evidence for [eveni n']. In
    this case, the inductive hypothesis says that the property we are
    trying to prove holds for [n']. *)

(** Let's try our current lemma again: *)

Lemma eveni_evend : forall n,
  eveni n -> evend n.
Proof.
  intros n E.
...
Theorem eveni_evend_iff : forall n,
  eveni n <-> evend n.
Proof.
  intros n. split.
  - (* -> *) apply eveni_evend.
  - (* <- *) unfold evend. intros [k Hk]. rewrite Hk. apply eveni_double.
Qed.
...
(** **** Exercise: 2 stars, standard (eveni_sum) *)
Theorem eveni_sum : forall n m, eveni n -> eveni m -> eveni (n + m).
Proof.
  intros n m En Em. induction En as [| n' En' IH].
  - simpl. apply Em. 
  - simpl. apply (eveni_SS (n'+m) IH).
Qed.
(** [] *)

(** **** Exercise: 4 stars, advanced, optional (eveni'_eveni)

    In general, there may be multiple ways of defining a
    property inductively.  For example, here's a (slightly contrived)
    alternative definition for [eveni]: *)

Inductive eveni' : nat -> Prop :=
| eveni'_0 : eveni' 0
| eveni'_2 : eveni' 2
| eveni'_sum n m (Hn : eveni' n) (Hm : eveni' m) : eveni' (n + m).
...
Theorem eveni'_eveni : forall n, eveni' n <-> eveni n.
Proof.
  intros n. split.
  - intros En. induction En as [| |n' m' En' IHn' Em' IHm'].
    + apply eveni_0.
    + apply (eveni_SS 0 eveni_0).
    + apply eveni_sum. apply IHn'. apply IHm'.
  - intros En. induction En as [| n' En' IHn'].
    + apply eveni'_0.
    + assert (H: S (S n') = 2 + n'). { reflexivity. } rewrite H.
      apply eveni'_sum.
      apply eveni'_2.
      apply IHn'.
Qed.
...
(** **** Exercise: 3 stars, advanced, especially useful (eveni_eveni__eveni)

    There are two pieces of evidence you could attempt to induct upon
    here. If one doesn't work, try the other. *)

Theorem eveni_eveni__eveni : forall n m,
  eveni (n+m) -> eveni n -> eveni m.
Proof.
  intros n m Enm En. induction En as [|n' En' IH].
  - apply Enm.
  - apply IH. simpl in Enm. apply evSS_eveni in Enm. apply Enm.
Qed. 
(** [] *)

(** **** Exercise: 3 stars, standard, optional (eveni_plus_plus)
...
Theorem eveni_plus_plus : forall n m p,
  eveni (n+m) -> eveni (n+p) -> eveni (m+p).
Proof.
  intros n m p Enm Enp. 
  assert (E: eveni (n+m+(n+p))).
  apply eveni_sum. apply Enm. apply Enp. 
  rewrite <- plus_swap in E. rewrite <- plus_assoc in E. rewrite plus_assoc in E.
  apply eveni_eveni__eveni with (n := n + n).
  apply E.
  rewrite <- double_plus.
  apply eveni_double.
Qed.
...
(** A proposition parameterized by a number (such as [eveni])
...
(** Proofs of facts about [<=] using the constructors [le_n] and
    ...
    [eveni] above. We can [apply] the constructors to prove [<=]
...
Inductive next_eveni : nat -> nat -> Prop :=
  | ne_1 n (H: eveni (S n))     : next_eveni n (S n)
  | ne_2 n (H: eveni (S (S n))) : next_eveni n (S (S n)).
...
Proof.
  intros m n o Emn Eno. induction Eno as [|o' Eno' IH'].
  + apply Emn.
  + apply le_S. apply IH'. 
Qed. 
...
Theorem O_le_n : forall n,
  0 <= n.
Proof.
  induction n. apply le_n. apply (le_S 0 n IHn).
Qed.
...
Proof.
  induction b.
  - Search (_ = _ + O). rewrite <- plus_n_O. apply le_n.
...
Theorem plus_le : forall n1 n2 m,
  ...
  n1 <= m /\ n2 <= m.
Proof.
  intros n1 n2 m E. induction E as [| m' E' IH].
  + split.
    - apply le_plus_l.
...
Proof.
  ...
  - intros m p q H. simpl in H. induction p.
    + simpl in H. right. assert (H': S n + m = S (n + m)). { reflexivity. }
      rewrite <- H' in H. apply plus_le in H. destruct H. apply H0. 
...
Theorem lt_S : forall n m,
  ...
  n < S m.
Proof.
  unfold lt. intros n m E. apply le_S. apply E.
Qed.
...
Theorem plus_lt : forall n1 n2 m,
  ...
  n1 < m /\ n2 < m.
Proof.
  intros n1 n2 m. unfold lt. intros. induction H. 
  - split.
...
Proof.
  ...
  apply leb_complete in Hnm. apply leb_complete in Hmo.
  apply leb_correct.
...
(** - Which of the following propositions are provable?
      ...
      The relationship is m + n = o.

      [R 0 0 0] -> c2[R 1 0 1] -> c2[R 1 1 2]
      [R 2 2 6] x 
...
Proof.
  ...
  - generalize dependent o. generalize dependent n. (*这里顺序很重要，因为之后induction n*)
    induction m.
    + simpl. induction n.
    ...
    + induction n.
      ...
      * intros. simpl in H. rewrite <- H. apply c3. apply IHn.
...
Inductive subseq : list nat -> list nat -> Prop :=
  | ss_1 l : subseq nil l
  | ss_2 h l1 l2 (H: subseq l1 l2) : subseq l1 (h :: l2)
  | ss_3 h l1 l2 (H: subseq l1 l2) : subseq (h :: l1) (h :: l2)
.
...
Theorem subseq_app : forall (l1 l2 l3 : list nat),
  ...
  subseq l1 (l2 ++ l3).
Proof.
  intros l1 l2 l3 E. induction E.
  - apply ss_1.
  - simpl. apply ss_2. apply IHE.
  - simpl. apply ss_3. apply IHE. 
Qed. 

(* https://github.com/StevenWongChess/software-foundations/blob/master/IndProp.v *)
Theorem subseq_trans : forall (l1 l2 l3 : list nat),
  subseq l1 l2 ->
  subseq l2 l3 ->
  subseq l1 l3.
Proof.
  intros. generalize dependent l1. induction H0. (* generalize dependent l1 是关键，不然最后一个 sub goal 会有问题*)
  - intros. inversion H. apply ss_1.
  - intros. apply ss_2. apply IHsubseq. apply H.
  - intros. inversion H.
    + apply ss_1.
    + apply ss_2. apply IHsubseq. apply H3.
    + apply ss_3. apply IHsubseq. apply H3.
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (R_provability2)
    ...
    - [R 2 [1;0]] 
      yes. c1[R 0 []] -> c2[R 1 [0]] -> c2[R 2 [1;0]]

    - [R 1 [1;2;1;0]]
      yes. c1[R 0 []] -> c2 x 4 -> [R 4 [1;2;1;0]] -> c3 x 3 -> [R 1 [1;2;1;0]]
...
(** The [eveni] property provides a simple example for
...
Proof.
  ...
  - simpl. split.
    + intros. inversion H. inversion H3. simpl. f_equal. apply IHs2. apply H4.
    + intros. rewrite H. assert (H': [x] ++ s2 = x :: s2). { reflexivity. } 
...
(** Something interesting happens in the [MApp] case.  We obtain
    ...
    destruct Hin as [Hin | Hin]. (* HIUDE: Why does  eqn:EHin  not work?? *)
    + (* In x s1 *)
      left. apply (IH1 Hin).
    + (* In x s2 *)
      right. apply (IH2 Hin).
...
Proof.
  ...
  - intros. induction re.
    ...
    + inversion H. Search (_ && _ = true). apply andb_true_iff in H1. destruct H1.
...
Proof.
  ...
  - injection Heqre' as Heqre''. exists []. split.
    * reflexivity.
    * intros. destruct H.
  - inversion Heqre'. rewrite H2 in *. destruct IHexp_match2 as [ss [L R]].
    * reflexivity.
    * exists (s1 :: ss). split.
      + simpl. f_equal. apply L.
      + intros. destruct H1.
        ** rewrite <- H1. apply H.
        ** apply R. apply H1.
Qed.
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | ns_1 : nostutter []
  | ns_2 x : nostutter [x]
  | ns_3 x y l (H1 : reflect (x = y) false) (H2 : nostutter (y :: l)) : nostutter (x :: y :: l)
 (* FILL IN HERE *)
.
...
repeat match goal with
  ...
  inversion H5. destruct H. reflexivity.
Qed.
...
Inductive in_order {X:Type} : list X -> list X -> list X -> Prop :=
  | io_1 : in_order [] [] []
  | io_2 x l1 l2 l (H: in_order l1 l2 l) : in_order (x :: l1) l2 (x :: l)
  | io_3 x l1 l2 l (H: in_order l1 l2 l) : in_order l1 (x :: l2) (x :: l)
.

Lemma in_order_lemma : forall X (test: X -> bool) x (l: list X),
  ~(filter test l = x :: l).
Proof.
Admitted.

Theorem in_order_test : forall X (test: X -> bool) (l1 l2 l: list X),
  filter test l1 = l1 ->
  filter test l2 = [] -> 
  in_order l1 l2 l -> 
  filter test l = l1.
Proof.
  intros. induction H1.
  - reflexivity.
  - simpl. destruct (test x) eqn:Ht.
    * simpl in H. rewrite Ht in H. f_equal. injection H as H.
      apply IHin_order. apply H. apply H0.
    * simpl in H. rewrite Ht in H. apply in_order_lemma in H. destruct H.
  - simpl in H0. destruct (test x) eqn:Ht.
    * discriminate H0.
    * simpl. rewrite Ht. apply IHin_order. apply H. apply H0.
Qed.
...
Proof.
  intros. induction l.
  - simpl. apply p1.
  - simpl. assert (H: (x :: l ++ rev l ++ [x]) = ([x] ++ l ++ rev l ++ [x])).
    { reflexivity. }
    rewrite H. rewrite (app_assoc _ l (rev l) [x]). apply p3. apply IHl.
Qed.
...
Proof.
  ...
  - reflexivity.
  - reflexivity.
  - simpl. rewrite rev_app_distr. simpl. f_equal. rewrite <- IHpal. reflexivity.
Qed. 
...
(* https://coq.discourse.group/t/is-it-possible-to-prove-the-theorem-i-created/553 *)
...
Proof.
  intros. apply rev_pal_lemma in H. destruct H as [H | H].
  - destruct H as [x [l' H]].
    rewrite H. apply p3. admit.
  - rewrite H. apply p1. Admitted.  
