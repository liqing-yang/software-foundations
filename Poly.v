...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite -> map_rev_lemma. simpl. rewrite -> IHl'. reflexivity. Qed.
...
(* check if all nats in a list are even *)
...
Proof.
  ...
  - reflexivity.
  - simpl. assert (H: fold_length (n :: l') = S (fold_length l')). { reflexivity. }
    rewrite -> H. rewrite -> IHl'. reflexivity. Qed.
(** [] *)
...
Proof.
  ...
  - reflexivity.
  - simpl. assert (H: fold_map f (x :: l') = f x :: fold_map f l'). { reflexivity. }
    rewrite -> H. rewrite -> IHl'. reflexivity. Qed.
