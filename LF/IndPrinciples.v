...
(** Inductive definitions of propositions also cause Coq to generate
    induction priniciples.  For example, recall our proposition [eveni],
    repeated here as [eveni'']: *)

Inductive eveni'' : nat -> Prop :=
| eveni_0 : eveni'' 0
| eveni_SS (n : nat) : eveni'' n -> eveni'' (S (S n)).

Check eveni''_ind :
  forall P : nat -> Prop,
    P 0 ->
    (forall n : nat, eveni'' n -> P n -> P (S (S n))) ->
    forall n : nat, eveni'' n -> P n.

(** In English, [eveni''_ind] says: Suppose [P] is a property of natural
...
(** As expected, we can apply [eveni''_ind] directly instead of using
    [induction].  For example, we can use it to show that [eveni'] (the
    ...
    cleaner inductive definition [eveni'']: *)
...
(** The induction principle that Coq generated for [eveni''] was parameterized
    ...
    forall P : (forall n : nat, eveni'' n -> Prop),
      P O eveni_0 ->
      (forall (m : nat) (E : eveni'' m),
        P m E -> P (S (S m)) (eveni_SS m E)) ->
      forall (n : nat) (E : eveni'' n), P n E
*)

(**   ... because:

     - Since [eveni''] is indexed by a number [n] (every [eveni''] object [E] is
...
(**  We can adapt this approach to proving [nat_ind] to help prove
    ...
    relating the [eveni] predicate we defined in [IndProp]
    to the [evenb] function defined in [Basics]. *)

Lemma evenb_eveni : forall n: nat, evenb n = true -> eveni'' n.
Proof.
  induction n; intros.
  - apply eveni_0.
  - destruct n.
...
(** Attempts to prove this by standard induction on [n] fail in the case for
    ...
    [Lemma evenb_eveni' : forall n : nat,
     (evenb n = true -> eveni n) /\ (evenb (S n) = true -> eveni (S n))].
...
Lemma evenb_eveni : forall n, evenb n = true -> eveni'' n.
Proof.
 intros.
