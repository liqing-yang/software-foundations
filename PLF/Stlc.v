...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | f_con : forall t1 t1' t2 t2' t3 t3',
      substi s x t1 t1' ->
      substi s x t2 t2' ->
      substi s x t3 t3' ->
...
Proof.
  ...
  eapply multi_step. simpl. apply ST_AppAbs. auto.
...
Proof.
  ...
  apply T_App with (T2:=Ty_Bool). apply T_Var. apply update_eq.
  apply T_App with (T2:=Ty_Bool). apply T_Var. apply update_eq.
  apply T_Var. apply update_neq. intros contra. discriminate.
Qed.
...
Proof.
  ...
  apply T_App with (T2:=Ty_Bool). apply T_Var. auto.
  apply T_App with (T2:=Ty_Bool). apply T_Var. auto.
