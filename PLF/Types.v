...
Proof.
  ...
  - unfold step_normal_form. intros [H1 H2]. inversion H2. inversion H0.
...
Lemma nv_impossbile : forall v,
  nvalue v -> ~(exists t, v --> t).
Proof.
  intros v H. induction H; intros [t' H'].
  - inversion H'.
  - apply IHnvalue. inversion H'. exists t1'. assumption.
Qed. 
...
Proof.
  intros t H. unfold step_normal_form. intros [t' H']. inversion H.
  - inversion H0; subst; inversion H'.
  - induction H0. 
    * inversion H'.
    * assert (Hv: nvalue (scc t)). constructor. assumption.
      apply nv_impossbile in Hv. apply Hv. exists t'. assumption.
Qed.
...
Proof with eauto.
  ...
  - inversion Hy2; subst. reflexivity. inversion H3.
  - inversion Hy2; subst. inversion Hy1. inversion Hy1. apply IHHy1 in H3. rewrite H3. reflexivity.
  - inversion Hy2. subst. apply IHHy1 in H0. rewrite H0. reflexivity.
  - inversion Hy2; subst. reflexivity. inversion H0.
  - inversion Hy2; subst. reflexivity. 
    assert (Hv: nvalue (scc v)). constructor. assumption. apply nv_impossbile in Hv. 
    exfalso. apply Hv. exists t1'. assumption.
  - inversion Hy2; subst. inversion Hy1. 
    assert (Hy: nvalue (scc y2)). constructor. assumption.
    apply nv_impossbile in Hy. exfalso. apply Hy. exists t1'. assumption.
    apply IHHy1 in H0. rewrite H0. reflexivity.
  - inversion Hy2. reflexivity. inversion H0.
  - inversion Hy2; subst. reflexivity. 
    assert (Hv : nvalue (scc v)). constructor. assumption.
    apply nv_impossbile in Hv. exfalso. apply Hv. exists t1'. assumption.
  - inversion Hy2; subst. inversion Hy1.
    assert (Hv : nvalue (scc v)). constructor. assumption.
    apply nv_impossbile in Hv. exfalso. apply Hv. exists t1'. assumption.
    apply IHHy1 in H0. rewrite H0. reflexivity.
...
(* Lemma prd_v : forall t,
  nvalue t -> nvalue (prd t).
Proof.
  intros t H. apply nat_canonical.
...
Proof.
  ...
  - destruct IHHT as [H1 | H2].
    * left. right. constructor. apply nat_canonical; assumption.
    * right. destruct H2 as [t' H]. exists (scc t'). constructor. assumption.
  - destruct IHHT as [H | H]. 
    * induction H.
      + inversion H; subst; inversion HT.
      + induction H.
        ++ right. exists zro. constructor.
        ++ right. exists t. constructor. assumption.
    * destruct H as [t' H]. right. exists (prd t'). constructor. assumption.
  - destruct IHHT as [H | H].
    * right. inversion H; inversion H0; subst; try (inversion HT).
      + exists tru. constructor.
      + exists fls. constructor. assumption.
    * destruct H as [t' H]. right. exists (iszro t'). constructor. assumption.
Qed.
...
Proof.
  ...
  induction HT;
    ...
    - inversion HE; subst. apply T_Scc. apply IHHT in H0. assumption.
    - inversion HE; subst.
      + constructor.
      + inversion HT. subst. assumption.
      + constructor. apply IHHT in H0. assumption.
...
Proof with eauto.
  ...
  induction HE; intros T HT; inversion HT; subst; auto.
...
(** **** Exercise: 2 stars, standard (variation1)
   ...
   counterexample.
      - Determinism of [step]
            remains true, as it did not modify the definition of step.
      - Progress
            becomes false. (scc tru) not a value & cannot take any steps
      - Preservation
            remains true.
*)
(* Do not modify the following line: *)
...
(** **** Exercise: 2 stars, standard, optional (variation3)
   ...
   this rule?  For each one that does, give a counter-example.
        - Determinisim 
          (test (iszro zro) (iszro zro) t3) --> (test tru (iszro zro) t3) By ST_Test
          (test (iszro zro) (iszro zro) t3) --> (test (iszro zro) tru t3) By ST_Funny2
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (variation5)
   ...
   this rule?  For each one that does, give a counter-example.
    - progress
        (test zro tru tru) is not a value & cannot take any steps
    - preservation
        !remains true.
        as there will be only one new case
        (test zro t1 t2) : T, but it cannot take any steps... 无法构成第二个assumption
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (variation6)
   ...
   - progress
    !remains true
      (test (prd zro) tru tru) is not a value but .. --> (test zro tru tru)
   - preservation
      (test (prd zro) tru tru) : Bool
      (test (prd zro) tru tru) --> (test zro tru tru)
      (test zro tru tru) is ill-typed.
*)
(** [] *)
...
(** **** Exercise: 1 star, standard (remove_prdzro)
    ...
    (test (iszro (prd zro)) tru tru): Bool
    but it's not a value & cannot take any steps
