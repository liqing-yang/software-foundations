...
Fixpoint type_check (Gamma : context) (t : tm) : option ty :=
  ...
  | <{ case t0 of | inl x1 => t1 | inr x2 => t2 }> => 
    ...
    | Some <{{T1 + T2}}> =>
      ...
      | Some T3, Some T4 => 
          if eqb_ty T3 T4 then Some T3 else None 
      | _,_ => None 
      end
    | _ => None 
    end
  ...
  | <{ h :: t }> => 
    ...
    match T1, T2 with 
    | <{{T}}>, <{{List T'}}> =>
      if eqb_ty T T' then Some <{{List T'}}> else None 
    | _,_ => None 
    end 
  ...
  | <{ fix t }> => 
    match type_check Gamma t with 
    | Some <{{T1->T1'}}> => 
      if eqb_ty T1 T1' then return T1 else fail
    | _ => fail 
    end 
  end.
...
Proof with eauto.
  ...
  - (* case *)
    ...
    fully_invert_typecheck Gamma t1 T1 T11 T12.
    invert_typecheck (x1 |-> T11; Gamma) t2 T2.
    invert_typecheck (x2 |-> T12; Gamma) t3 T3.
    case_equality T2 T3.
  - (* nil *)
    rename t into T'. eauto.
  - (* cons *)
    rename t1 into h, t2 into t.
    invert_typecheck Gamma h T1.
