...
(** **** Exercise: 1 star, standard, optional (valid_triples)
   ...
   3) {{True}} X := 5; Y := 0 {{X = 5}} yes

   4) {{X = 2 /\ X = 3}} X := 5 {{X = 0}} yes

   5) {{True}} skip {{False}} no

   6) {{False}} skip {{True}} yes

   7) {{True}} while true do skip end {{False}} no
...
Theorem hoare_post_true : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P Q c st. unfold hoare_triple. intros. apply st.
Qed.
...
Proof.
  ...
  - inversion H. subst. rewrite t_update_shadow. destruct H0. rewrite <- H1. 
...
Theorem hoare_asgn_fwd_exists :
  ...
  {{fun st => exists m, P (X !-> m ; st) /\
                st X = aeval (X !-> m ; st) a }}.
Proof.
  intros. unfold hoare_triple. intros. exists (st X). split.
  - inversion H. subst. rewrite t_update_shadow. rewrite t_update_same. assumption.
