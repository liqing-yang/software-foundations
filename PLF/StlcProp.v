...
Proof.
  ...
  - right. inversion Ht. subst. apply IHt1 in H2. apply IHt2 in H4.
    destruct H2 as [Hv1 | H2].
    + destruct H4 as [Hv2 | H4].
      * inversion Ht. subst. assert (H: exists x u, t1 = <{\x:T0, u}>).
        { eapply canonical_forms_fun. apply H2. assumption. }
        destruct H as [x [u H]]. rewrite H. 
        exists <{ [x:=t2]u }>. constructor. assumption.
      * destruct H4 as [t2' H]. exists <{ t1 t2' }>.
        constructor; assumption.
    + destruct H2 as [t1' H]. exists <{ t1' t2 }>.
      constructor; assumption.
  - right. inversion Ht. subst. 
    apply IHt1 in H3. inversion Ht. subst. clear H8. clear H9.
    destruct H3 as [H | H].
    + assert (Hbool: (t1 = <{true}>) \/ (t1 = <{false}>)).
      { apply canonical_forms_bool; assumption. }
      destruct Hbool; subst.
      * exists t2. constructor.
      * exists t3. constructor.
    + destruct H as [t1' H]. exists <{ if t1' then t2 else t3 }>.
      constructor; assumption.
Qed.
(** [] *)
...
Proof.
  intros Gamma t T.
  eapply weakening. (*可以unfold inclusion看看为什么要用discriminate*)
  discriminate.
Qed.
...
Proof.
  ...
  - rename x0 into y. destruct (eqb_stringP x y); subst.
    + rewrite update_eq in H. injection H as H. rewrite <- H.
      apply weakening_empty. assumption.
    + rewrite update_neq in H; auto.
  - rename x0 into y, T2 into S. destruct (eqb_stringP x y); subst.
    + apply T_Abs. rewrite update_shadow in Ht. assumption.
...
(* t' = true
   |- t' \in Bool
   t = if true then (\x: Bool, x) else true
   t --> t'
   ~(|- t \in Bool)
...
Proof.
  ...
  induction Hmulti.
  - apply progress in Hhas_type. destruct Hhas_type as [H | H]; auto.
  - apply IHHmulti; auto.
...
Proof.
  ...
  induction H; intros T' H'; inversion H'; subst; auto.
  - rewrite H in H2. injection H2. trivial.
  - apply IHhas_type in H5. rewrite H5. reflexivity.
  - apply IHhas_type1 in H4. inversion H4. reflexivity.
Qed.
(** [] *)
...
(* 
                        ...
                        -----------------------------                    (F_App2)
                         ...
                         ----------------------                          (F_Abs)
                           ...
                           x \freein t1
                         ---------------------                          (F_IF1)
                         x \freein if t1 then t2 else t3
...
Proof.
  ...
  rename y0 into y. rename IHappears_free_in into IH.
  inversion H1. subst. apply IH in H7. destruct H7 as [T' H'].
  exists T'. rewrite update_neq in H'; assumption. (*记得 y <> x 很重要*)
Qed.
(** [] *)
...
Corollary typable_empty__closed : forall t T,
    ...
    closed t.
Proof.
  unfold closed. intros t T Ht x Hf.
  apply (free_in_context) with (T:=T) (Gamma:=empty) in Hf.
  - destruct Hf. discriminate.
  - assumption.
Qed.
...
Proof.
  ...
  - constructor. apply IHhas_type. intros x' Hf'. 
    destruct (eqb_stringP x0 x'); subst.
    + rewrite update_eq. rewrite update_eq. reflexivity.
    + rewrite update_neq. rewrite update_neq. apply H0. constructor; assumption.
      assumption. assumption.
  - apply T_App with (T2:=T2); auto.
Qed.
(** [] *)
...
Theorem stlc_progress : forall t T,
  empty |- t \in T ->
  (value t) \/ (exists t', t --> t').
Proof.
Admitted.

Theorem stlc_preservation : forall t T Gamma t',
  Gamma |- t \in T -> 
  t --> t' -> 
  Gamma |- t' \in T.
Proof. 
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
      肯定false了。。。现在根本不会出现stuck的情况了。。。zap --> zap --> zap 无限下去都可以
      - Progress
      remains true，因为永远存在t'使t --> t'。。。
      - Preservation
      zap的类型可以是Bool->Bool，还可以是Bool。。。肯定false了
*)
...
(** **** Exercise: 2 stars, standard (stlc_variation2)
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
      remains true。。
      因为本来abstraction cannot take any steps，新为它加一个rule并不会造成冲突
      foo 也只有一种 step rule
      - Progress
      remains true 没啥好说的 没有新增 type rule
      - Preservation
      false 
      t = \x : Bool, x
      |- t \in (Bool -> Bool)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation6)
    ...
    false, give a counterexample.
      ...
      false 
      t = true true
      t is not a value and it cannot take any steps
      - Preservation
      不影响 因为 t-->t' 根本不可能
*)
(** [] *)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation7)
    ...
    false, give a counterexample.
      ...
      - Progress
      remains true
      因为 abstraction 一定是 value
      - Preservation
      remains true? 不太确定 试了很久都没试出来counterexample
      假如 t 是 \x: Bool, t1 的话。。无论t是什么，preservation still holds
*)
(** [] *)
