...
Proof.
  exists (P (P (C 1) (C 2)) (C 3)). split.
  - constructor.
  - unfold normal_form, not. intros H. apply H.
    exists (P (C 3) (C 3)).
...
Proof.
  exists (C 1). split.
  - constructor.
  - unfold normal_form, not. intros. apply H. exists (P (C 1) (C 0)).
    constructor.
Qed.
...
Proof.
  ...
  - unfold normal_form. intros [t H]. inversion H. subst. inversion H3.
Qed.  
...
Proof.
  ...
  - destruct H as [t1' H]. exists (test t1' t2 t3).
...
(** **** Exercise: 3 stars, standard, optional (properties_of_altered_step)
    ...
    - Is the [step] relation still deterministic?  Write yes or no and
      ...
      NO! 
      (test t1 t2 t2) with t1 --> t1'
      origin (test t1 t2 t2) --> (test t1' t2 t2)
      now    (test t1 t2 t2) --> t2
...
Theorem step_deterministic :
  ~ deterministic step.
Proof.
  unfold not, deterministic.
  intros. 
  assert (contra : (test fls tru tru = tru)).
  apply H with (x:= (test (test tru fls fls) tru tru)); repeat constructor.
  discriminate.
Qed.
...
Proof.
  ...
  - unfold step_normal_form, normal_form in P12. inversion P21; subst.
    + reflexivity.
    + exfalso. apply P12. exists y. assumption.
  - apply IHP11; try assumption. inversion P21; subst.
    + unfold step_normal_form, normal_form in P22.
      exfalso. apply P22. exists y. assumption.
    + assert (H2: y = y0). eapply step_deterministic. apply H. apply H0.
      subst. assumption.
Qed.
(** [] *)
...
Lemma multistep_congr_2 : forall t1 t2 t2',
     ...
     P t1 t2 -->* P t1 t2'.
Proof.
  intros t1 t2 t2' v1 H. induction H.
  - apply multi_refl.
  - eapply multi_step. { apply ST_Plus2. apply v1. apply H. }
    apply IHmulti.
Qed.
(** [] *)
  ...
  - inversion H. subst. apply IHt1 in H2. apply IHt2 in H4.
    apply multi_trans with (y:=P (C n1) t2). 
      ...
      + apply multi_trans with (y:= P (C n1) (C n2)).
        * apply multistep_congr_2. apply v_const. apply H4.
...
Proof.
  ...
  induction Hs; intros.
  - inversion H. subst. apply E_Plus; apply E_Const.
  - inversion H. subst. apply E_Plus.
    * apply IHHs. assumption.
    * assumption.
  - inversion H0. subst. apply E_Plus.
    * assumption.
    * apply IHHs. assumption.
Qed.
(** [] *)
...
Proof.
  unfold normal_form_of. intros t t' [H1 H2]. induction H1.
  - apply nf_is_value in H2. inversion H2. exists n. split.
    + reflexivity.
    + apply E_Const.
  - apply IHmulti in H2. destruct H2 as [n [H2 H3]]. exists n. split.
    + assumption.
...
Proof.
  intros n st [H1 H2]. unfold par_loop.
  eapply multi_step. apply CS_Par2. apply CS_While. 
  eapply multi_step. apply CS_Par2. apply CS_IfStep. 
  apply BS_Eq1. apply AS_Id.
  eapply multi_step. apply CS_Par2. apply CS_IfStep. rewrite H2. apply BS_Eq. simpl.
  eapply multi_step. apply CS_Par2. apply CS_IfTrue.
  eapply multi_step. apply CS_Par2. apply CS_SeqStep. apply CS_AssStep. 
  apply AS_Plus1. apply AS_Id. rewrite H1. 
  eapply multi_step. apply CS_Par2. apply CS_SeqStep. apply CS_AssStep.
  apply AS_Plus.
...
Proof.
  induction n as [|n' IHn']; try intros st [Hx Hy]; unfold par_loop.
  - exists st. split. apply multi_refl. split; assumption.
  - assert (H : st X = 0 /\ st Y = 0). split; assumption.
    apply IHn' in H. destruct H as [st' [H' [Hx' Hy']]].
    exists (t_update st' X (S n')). split.
    + eapply multi_trans. apply H'. apply par_body_n__Sn. split;assumption.
    + split.
      * rewrite t_update_eq. reflexivity.
      * rewrite t_update_neq. assumption.
        destruct (eqb_string X Y) eqn:Hxy.
        ++ inversion Hxy.
...
Proof.
  ...
  - eapply multi_step. apply ST_Plus2. constructor.
    apply ST_PlusConstConst. simpl. 
    eapply multi_step. apply ST_PlusConstConst. simpl.
    apply multi_refl.
  - constructor.
Qed.
(** [] *)
...
Proof.
  exists (C 6). split.
  - apply multi_step with (P (C 3) (C (2 + 1))).
    apply ST_Plus2. constructor. apply ST_PlusConstConst. simpl.
    apply multi_step with (C (3 + 3)). apply ST_PlusConstConst.
    apply multi_refl.
  - constructor.
Qed.
(** [] *)
