...
(** **** Exercise: 1 star, standard, optional (fact_steps)
    ...
    [-->] [ST_FixAbs]
        (\x. if x=0 then 1 else x * ((fix (\f \x. if x=0 then 1 else x * (f (pred x))))) (pred x))) 1
    [-->] [ST_AppAbs]
        if 1=0 then 1 else 1 * ((fix (\f \x. if x=0 then 1 else x * (f (pred x))))) (pred 1))
    [-->]
        1 * ((fix (\f \x. if x=0 then 1 else x * (f (pred x))))) (pred 1))
    [-->] [ST_FixAbs]
        1 * ((\x. if x=0 then 1 else x * (fix (\f \x. if x=0 then 1 else x * (f (pred x))) (pred x))) (pred 1))
    [-->] [ST_PredNat]
        1 * ((\x. if x=0 then 1 else x * (fix (\f \x. if x=0 then 1 else x * (f (pred x))) (pred x))) 0)
    [-->] [ST_AppAbs]
        1 * (if 0=0 then 1 else 1 * (fix (\f \x. if x=0 then 1 else x * (f (pred x))) (pred 0)))
    [-->]
        1 * 1
    [-->]
        1
...
Fixpoint subst (x : string) (s : tm) (t : tm) : tm :=
  ...
  (* let *)
  | <{ let y = t1 in t2 }> => if (eqb_string x y) then <{ let y = ([x:=s]t1) in t2 }>
...
Proof with eauto.
  ...
  - (* T_Let *)
    ...
    (* fix *)
  - (* T_Fix *)
    right. destruct IHHt. reflexivity.
    + (* ST_FixAbs *)
...
Proof with eauto.
  ...
  - (* tm_let *)
    rename s into y. destruct (eqb_stringP x y); subst.
    + (* x=y *)
      apply T_Let with (T1:=T1)... rewrite update_shadow in H6...
    + (* x<>y *)
      apply T_Let with (T1:=T1)... rewrite update_permute in H6...
Qed. 
...
Proof with eauto.
  ...
  (* let *)
  - (* T_Let *)
    rename x0 into x. eapply substitution_preserves_typing...
