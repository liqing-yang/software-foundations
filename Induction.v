...
Proof.
  ...
  - simpl. assert (H: S m = 1 + m). { reflexivity. }
    rewrite -> H. rewrite -> mult_n_Sm. rewrite -> IHm'. reflexivity. Qed.
...
Example test_btn: bin_to_nat (B1 (B0 (B0 Z))) = 1.
Proof.
...
Example normalize_test1 : normalize (B0 (B0 Z)) = Z.
Proof.
  simpl. reflexivity. Qed.

Definition bin2 (b : bin) : bin :=
  match b with 
  | Z => Z 
  | B0 b' => B0 (B0 b')
  | B1 b' => B0 (B1 b')
...
Lemma lemma12 : forall b, incr (incr (bin2 b)) = bin2 (incr b).
Proof.
  destruct b.
  - reflexivity.
  - simpl. reflexivity.
...
Lemma lemma1 : forall n, nat_to_bin (2 * n) = bin2 (nat_to_bin n).
Proof.
  induction n as [| n' IHn'].
  - reflexivity.
  - simpl. 
    assert (H: incr (nat_to_bin (n' + S (n' + 0))) = incr (incr (nat_to_bin (2 * n')))).
...
Lemma lemma2 : forall b, bin2 (normalize b) = normalize (B0 b).
Proof.
  destruct b.
  - reflexivity.
  - simpl. reflexivity.
...
Lemma lemma3 : forall b, incr (bin2 b) = B1 b.
Proof.
  destruct b.
  - reflexivity.
  - simpl. reflexivity.
...
Theorem bin_nat_isomo : forall b, nat_to_bin (bin_to_nat b) = normalize b.
Proof.
  induction b as [| b0' IHb0' | b1' IHb1'].
  - reflexivity.
  - assert (H: bin_to_nat (B0 b0') = 2 * bin_to_nat b0'). { reflexivity. }
    rewrite -> H.
    rewrite -> lemma1.
    rewrite -> IHb0'.
    rewrite -> lemma2.
    reflexivity.
  - assert (H: nat_to_bin (bin_to_nat (B1 b1'))= incr (nat_to_bin (2 * bin_to_nat b1'))).
    { reflexivity. }
    rewrite -> H.
    rewrite -> lemma1.
    simpl.
